...
<body>
    ...
    <table style="width:100%;">
        ...
        <tbody style="font-size:12px;text-align:center;">
            @foreach($user_clocking as $clocking)
             <tr>
                <td>{{$clocking->user}}</td>
                <td>{{$clocking->stage}}</td>
                <td>{{$clocking->start}}</td>
                <td>{{$clocking->end}}</td>
                <td>{{number_format((strtotime($clocking->end)-strtotime($clocking->start))/3600,2)}} Hrs.</td>
                <td>{{$clocking->Key_Ref}}</td>
             </tr>   
            @endforeach
        </tbody>
    </table>
