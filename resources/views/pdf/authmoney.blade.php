    ...
    <img src="img/letter_head.png">
        ...
        <table style="font-size:7;width:520">
           ...
           @foreach($quotations as $quotes)
            ...
            @php $count++;$overallPaint=$overallPaint+($quotes->Paint * $paintRate);$overalLabour=$overalLabour+($quotes->Labour * $laborRate);$overallStrip=$overallStrip+($quotes->Strip * $stripRate); $overallOutwork+=$quotes->Misc;$overallFrame+=$quotes->Frame * $frameRate;$parts=($quotes->Quantity*($quotes->Part +($quotes->Part * ($quotes->Percent/100))))+$parts;@endphp
            @endforeach
