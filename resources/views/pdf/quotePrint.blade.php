...
<body>
    ...
    <img src="img/letter_head.png">
        <p style="text-align:center;font-size:8"><b>ESTIMATE QUOTE</b><br> <span style="font-size:7">Date: {{date('Y-m-d')}}</span></p>
        ...
        <table style="font-size:7;width:520">
           ...
           @foreach($quotations as $quotes)
            <tr style="font-size:6.5;"><td style="text-align:left;width:20;">{{$count}}</td>
                ...
                <td style="text-align:right">{{number_format($quotes->Paint,2)}}</td>
                <td style="text-align:right">{{number_format($quotes->Labour,2)}}</td>
                <td style="text-align:right">{{number_format($quotes->Strip,2)}}</td>
                <td style="text-align:right">{{number_format($quotes->Frame,2)}}</td>
                <td style="text-align:right">{{number_format($quotes->Misc,2)}}</td>
            </tr>
            @php $count++;$parts=($quotes->Quantity*($quotes->Part +($quotes->Part * ($quotes->Percent/100))))+$parts;@endphp
            @endforeach
            <tfoot><tr><td></td></tr><tr><td style="text-align:left;font-size:7;width:26;"><b>Total</b></td>
                ...
                <td style="text-align:right;font-size:6"><b>{{number_format($paint_total * $paintRate,2)}}</b></td>
                <td style="text-align:right;font-size:6"><b>{{number_format($labour_total * $laborRate,2)}}</b></td>
                <td style="text-align:right;font-size:6"><b>{{number_format($strip_total * $stripRate,2)}}</b></td>
                <td style="text-align:right;font-size:6"><b>{{number_format($frame_total * $frameRate,2)}}</b></td>
                <td style="text-align:right;font-size:6"><b>{{number_format($outwork_total,2)}}</b></td>
            </tr></tfoot>  
                    ...
                    <td style="width:150">
                        <table>
                            ...
                            <tr>
                                ...
                                <td style="text-align:right;width:60">R {{number_format($covid,2)}}</td>
                            </tr>

                            <tr>
                                <td style="width:140;"><b>Sub Total </b></td>
                                <td style="text-align:right;width:60;"><b>R {{number_format(($labour_total*$laborRate)+($paint_total*$paintRate)+($strip_total*$stripRate)+($frame_total*$frameRate)+($parts*($shopSupply/100))+$outwork_total+($paint_total*$paintRate*($paintSupply/100))+$parts+$waste+$covid,2)}}</b></td>
                            </tr>
