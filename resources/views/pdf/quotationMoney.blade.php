...
<body>
    ...
    <img src="img/letter_head.png">
        ...
        <table style="font-size:7">
            ...
            @php $laborRate=$insurer->labour; $paintRate=$insurer->Paint;$stripRate=$insurer->Strip;$frameRate=$insurer->Frame;$paintSupply=$insurer->PaintSup;$shopSupply=$insurer->ShopSup;$waste=$waste_total;$covid=$covid_total;@endphp
            @endforeach
        ...
        <table style="font-size:7;width:520">
           ...
           @foreach($quotations as $quotes)
            <tr style="font-size:6.5;"><td style="text-align:left;width:20;">{{$count}}</td>
                ...
                <td style="text-align:right">{{number_format(($quotes->Part*$quotes->Quantity) +($quotes->Part * ($quotes->Percent/100)),2)}}</td>
            ...
            @php $count++;$overallPaint=$overallPaint+($quotes->Paint * $paintRate);$overallFrame+=$quotes->Frame * $frameRate;$overalLabour=$overalLabour+($quotes->Labour * $laborRate);$overallStrip=$overallStrip+($quotes->Strip * $stripRate); $overallOutwork+=($quotes->Misc);$parts=($quotes->Quantity*($quotes->Part +($quotes->Part * ($quotes->Percent/100))))+$parts;@endphp
            @endforeach
                    ...
                    <td style="width:150">
                        <table>
                            ...
                            <tr>
                                ...
                                <td style="text-align:right;width:60;"><b>R {{number_format($subtotal,2)}}</b></td>
                            </tr>
                            <tr>
                                <td style="width:140">VAT @ 15% </td>
                                <td style="text-align:right;width:60">R {{number_format(((($labour_total*$laborRate)+($paint_total*$paintRate)+($strip_total*$stripRate)+($frame_total*$frameRate)+($parts*($shopSupply/100))+$outwork_total+($paint_total*$paintRate*($paintSupply/100)) +$parts+$waste)*0.15),2)}}</td>
                            </tr>
                            <tr>
