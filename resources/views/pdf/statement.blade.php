...
@foreach($branch_invoice as $branch_inv)
<tr>
<td>{{$branch_inv->date_invoiced}}</td>
<td>PIC{{$branch_inv->id}}</td>
<td>Advanced Intelligence System</td>   
<td>R{{number_format(($branch_inv->total_price) + ($branch_inv->total_price*0.15),2)}}</td>
<td></td>
</tr>
@php $total_due+=$branch_inv->total_price + ($branch_inv->total_price*0.15); @endphp
